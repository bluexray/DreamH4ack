...
namespace DH.Core.DynamicProxy
{
    ...
    ///     创建动态类缓存
    /// </summary>
    public class DynamicsClassTypeCacheManger : CacheManger<int, Type>
    {
        /// <summary>
        ///     线程锁
        /// </summary>
        ...
        private DynamicsClassTypeCacheManger(Dictionary<string, Type> propertys, Type parentType = null) : base(0)
        {
            ...
            SafeCheck.IsTure(propertys == null, "propertys参数不能为空或为0");
        ...
        protected override Type SetCacheLock()
        {
            lock (LockObject)
            {
                ...
                //缓存中没有找到，新建一个构造函数的委托
                var val = _isListProperty ? Common.Dynamics.CreateClassType(_lstPropertys, _constructors, _parentType) : Dynamics.CreateClassType(_dicPropertys, _parentType);
